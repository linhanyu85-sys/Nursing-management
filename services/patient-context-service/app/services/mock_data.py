...
def _case_store_path() -> Path:
    return _project_root() / settings.mock_case_store_path
...
def _seed_state() -> tuple[dict[str, PatientBase], list[BedOverview], dict[str, PatientContextOut]]:
    patients = {
    ...
    beds.sort(key=lambda item: _bed_sort_key(item.bed_no))
    contexts = {
    ...
    return patients, beds, contexts


def _clone_patient(patient: PatientBase) -> PatientBase:
    return patient.model_copy(deep=True)


def _clone_bed(bed: BedOverview) -> BedOverview:
    return bed.model_copy(deep=True)


def _clone_context(context: PatientContextOut) -> PatientContextOut:
    return context.model_copy(deep=True)


def _apply_state(
    patients: dict[str, PatientBase],
    beds: list[BedOverview],
    contexts: dict[str, PatientContextOut],
) -> None:
    MOCK_PATIENTS.clear()
    MOCK_PATIENTS.update({patient_id: _clone_patient(item) for patient_id, item in patients.items()})
    MOCK_BEDS[:] = sorted([_clone_bed(item) for item in beds], key=lambda item: _bed_sort_key(item.bed_no))
    MOCK_CONTEXTS.clear()
    MOCK_CONTEXTS.update({patient_id: _clone_context(item) for patient_id, item in contexts.items()})


def _serialize_state() -> dict[str, Any]:
    return {
        "patients": [item.model_dump(mode="json") for item in MOCK_PATIENTS.values()],
        "beds": [item.model_dump(mode="json") for item in MOCK_BEDS],
        "contexts": [item.model_dump(mode="json") for item in MOCK_CONTEXTS.values()],
    }


def _save_case_state() -> None:
    path = _case_store_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(_serialize_state(), ensure_ascii=False, indent=2), encoding="utf-8")


def _load_case_state() -> None:
    path = _case_store_path()
    if not path.exists():
        patients, beds, contexts = _seed_state()
        _apply_state(patients, beds, contexts)
        return

    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
        patients = {
            item["id"]: PatientBase.model_validate(item)
            for item in raw.get("patients", [])
            if isinstance(item, dict) and item.get("id")
        }
        beds = [BedOverview.model_validate(item) for item in raw.get("beds", []) if isinstance(item, dict)]
        contexts = {
            item["patient_id"]: PatientContextOut.model_validate(item)
            for item in raw.get("contexts", [])
            if isinstance(item, dict) and item.get("patient_id")
        }
        if not patients or not beds or not contexts:
            raise ValueError("mock_case_store_incomplete")
        _apply_state(patients, beds, contexts)
    except Exception:
        patients, beds, contexts = _seed_state()
        _apply_state(patients, beds, contexts)
    ...
    for bed in MOCK_BEDS:
        ...
        if bed_copy.current_patient_id:
            ...
            if context is not None:
                ...
                bed_copy.patient_name = context.patient_name or bed_copy.patient_name
        result.append(bed_copy)
    return result


def _find_bed_index(bed_no: str) -> int | None:
    target = str(bed_no or "").strip()
    for index, bed in enumerate(MOCK_BEDS):
        if str(bed.bed_no).strip() == target:
            return index
    return None


def _occupied_bed_by_other(bed_no: str, patient_id: str) -> bool:
    index = _find_bed_index(bed_no)
    if index is None:
        return False
    current_patient_id = MOCK_BEDS[index].current_patient_id
    return bool(current_patient_id and current_patient_id != patient_id)


def _build_case_bundle(patient_id: str, created: bool) -> PatientCaseBundleOut | None:
    patient = MOCK_PATIENTS.get(patient_id)
    context = MOCK_CONTEXTS.get(patient_id)
    if patient is None or context is None:
        return None

    bed = next((item for item in MOCK_BEDS if item.current_patient_id == patient_id), None)
    if bed is None and context.bed_no:
        bed = next((item for item in MOCK_BEDS if item.bed_no == context.bed_no), None)
    if bed is None:
        return None

    return PatientCaseBundleOut(
        created=created,
        patient=patient.model_copy(deep=True),
        context=context.model_copy(deep=True),
        bed=bed.model_copy(deep=True),
    )


def get_patient_case_bundle(patient_id: str) -> PatientCaseBundleOut | None:
    return _build_case_bundle(patient_id, created=False)


def upsert_patient_case(payload: PatientCaseUpsertRequest) -> PatientCaseBundleOut:
    patient_id = payload.patient_id or f"pat-{uuid4().hex[:8]}"
    bed_no = str(payload.bed_no or "").strip()
    if not bed_no:
        raise ValueError("bed_no_required")
    if _occupied_bed_by_other(bed_no, patient_id):
        raise ValueError("bed_already_occupied")

    existing_patient = MOCK_PATIENTS.get(patient_id)
    existing_context = MOCK_CONTEXTS.get(patient_id)
    created = existing_patient is None
    previous_bed_no = str(existing_context.bed_no).strip() if existing_context and existing_context.bed_no else ""

    mrn = (payload.mrn or (existing_patient.mrn if existing_patient else "")).strip() or f"MRN-{uuid4().hex[:6].upper()}"
    encounter_id = (payload.encounter_id or (existing_context.encounter_id if existing_context else "")).strip() or f"enc-{uuid4().hex[:8]}"

    patient = PatientBase(
        id=patient_id,
        mrn=mrn,
        inpatient_no=payload.inpatient_no or (existing_patient.inpatient_no if existing_patient else None),
        full_name=payload.full_name.strip(),
        gender=payload.gender or (existing_patient.gender if existing_patient else None),
        age=payload.age if payload.age is not None else (existing_patient.age if existing_patient else None),
        blood_type=payload.blood_type or (existing_patient.blood_type if existing_patient else None),
        allergy_info=payload.allergy_info if payload.allergy_info is not None else (existing_patient.allergy_info if existing_patient else None),
        current_status=payload.current_status or (existing_patient.current_status if existing_patient else "admitted"),
    )
    context = PatientContextOut(
        ...
        diagnoses=[item.strip() for item in payload.diagnoses if item and item.strip()],
        risk_tags=[item.strip() for item in payload.risk_tags if item and item.strip()],
        pending_tasks=[item.strip() for item in payload.pending_tasks if item and item.strip()],
        latest_observations=[
            {
                "name": item.name,
                "value": item.value,
            ...
            for item in payload.latest_observations
            if item.name.strip() and item.value.strip()
        ],
        updated_at=datetime.now(timezone.utc),
    )

    if previous_bed_no and previous_bed_no != bed_no:
        previous_index = _find_bed_index(previous_bed_no)
        if previous_index is not None:
            MOCK_BEDS[previous_index] = BedOverview(
                id=MOCK_BEDS[previous_index].id,
                department_id=MOCK_BEDS[previous_index].department_id,
                bed_no=MOCK_BEDS[previous_index].bed_no,
                room_no=MOCK_BEDS[previous_index].room_no,
                status="vacant",
    ...
    target_index = _find_bed_index(bed_no)
    if target_index is None:
        ...
        current_bed = MOCK_BEDS[target_index]
        MOCK_BEDS[target_index] = BedOverview(
            id=current_bed.id,
            department_id=current_bed.department_id,
            bed_no=current_bed.bed_no,
            room_no=(payload.room_no or "").strip() or current_bed.room_no,
            status="occupied",
            current_patient_id=patient_id,
            patient_name=patient.full_name,
            risk_tags=list(context.risk_tags),
            pending_tasks=list(context.pending_tasks),
            latest_document_sync=current_bed.latest_document_sync,
        )

    MOCK_BEDS.sort(key=lambda item: _bed_sort_key(item.bed_no))
    MOCK_PATIENTS[patient_id] = patient
    MOCK_CONTEXTS[patient_id] = context
    _save_case_state()

    bundle = _build_case_bundle(patient_id, created=created)
    if bundle is None:
        raise ValueError("patient_case_build_failed")
    return bundle
...
def _build_orders() -> tuple[dict[str, list[OrderOut]], dict[str, OrderOut], dict[str, list[OrderOut]]]:
    ...
    for patient_id in MOCK_PATIENTS:
        ...
        history_by_patient.setdefault(patient_id, [])
