...
> 当前 GitHub 管理仓库聚焦后端服务、运维脚本和管理端骨架，不包含移动端独立仓库与本地大模型文件。
...
**开发中/待完善：**
- Admin 管理端已具备账号同步、病例编辑、病区分析看板与联通监控，仍可继续补充更细的审核流程
...
apps/
  ...
  admin-web/       # 管理后台（账号同步 / 病例编辑 / 病区分析 / 联通监控）
...
.\scripts\start_project.ps1 -NodeRoot "D:\软件\node" -Port 8094
```

如果你还要把本地模型一起拉起来：

```powershell
.\scripts\start_project.ps1 -NodeRoot "D:\软件\node" -Port 8094 -WithLocalModel
```

启动完成后常用地址：

- 后端健康检查：`http://127.0.0.1:8000/health`
- Expo Web 预览：`http://127.0.0.1:8094`
...
### 2. 启动基础设施（Docker）
```powershell
.\scripts\bootstrap_local_stack.ps1
```
...
```powershell
.\scripts\start_mobile_expo.ps1 -NodeRoot "D:\软件\node" -Port 8094
```

## 安装包位置

当前仓库里默认**不会自动保存 APK 文件**。我已经检查过，项目目录下现在没有现成的 `.apk` 或 `.aab` 安装包。

这个项目当前走的是 `EAS Build`：

- 执行 [D:\codex\project\scripts\build_apk.ps1](D:\codex\project\scripts\build_apk.ps1) 后
- Expo/EAS 会在命令行里返回一个下载链接
- 你需要从那个链接下载 APK

也就是说：

- 当前安装包位置：**还没有本地 APK 文件**
- 真正生成后的位置：**Expo/EAS 云端构建产物链接**
- 如果你下载到本机，建议放到项目根目录下自建的 `build\` 文件夹里统一管理
